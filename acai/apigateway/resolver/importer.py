...
class ResolverImporter:
    ...
    def get_handlers_path_abs(self):
        return self.file_separator + self.get_project_root() + self.file_separator + self.get_handlers_root()
    ...
    def get_handlers_file_tree(self):
        if not self.__handlers_tree:
            ...
            file_paths = [item.replace(self.get_handlers_path_abs() + self.file_separator, '') for item in file_list]
    ...
    def __get_glob_pattern(self):
        if '*' in self.__handlers and '.py' in self.__handlers:
            return self.file_separator + self.get_project_root() + self.file_separator + self.handlers
        return self.get_handlers_path_abs() + self.file_separator + '**' + self.file_separator + '*.py'
