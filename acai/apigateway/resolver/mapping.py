...
class Mapping:
    ...
    def __init__(self, **kwargs):
        self.__importer = ResolverImporter(handlers='/', mode='mapping')
    ...
    def _get_file_and_import_path(self, request_path):
        ...
        import_path = self.__importer.clean_path(mapping_file_path).replace(self.__importer.file_separator, '.').replace('.py', '')
        return file_path, import_path

    def __clean_mapping_key(self, mapping_path_key):
        base_path = [mapping_key for mapping_key in self.__handler_mapping if self.__base_path in mapping_key]
        if base_path:
            mapping_path_key = f'{self.__base_path}/{mapping_path_key}'
    ...
    def __get_abs_file_path(self, mapping_file_path):
        clean_path = self.__importer.clean_path(mapping_file_path)
        base_dir = clean_path.split(self.__importer.file_separator)[0]
        project_root = self.__importer.project_root.split(base_dir)[0]
        join_list = ['', self.__importer.clean_path(project_root), self.__importer.clean_path(mapping_file_path)]
        return f'{self.__importer.file_separator}'.join(join_list)
    ...
    def __get_matching_path_from_mapping(self, path_list):
        ...
        if len(matching_path) != 1 or len(matching_path) != len(path_list):
            raise ApiException(code=404, message='route not found')
        return f'{self.__importer.file_separator}'.join(matching_path)
    ...
    def __clean_route_mapping(self, route):
        mapping_key = route.replace(self.__base_path, '')
        return self.__importer.clean_path(mapping_key)
