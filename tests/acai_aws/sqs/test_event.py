...
from acai_aws.sqs.event import Event
...
from tests.mocks.sqs.mock_data_class import MockSQSDataClass


class SQSEventTest(unittest.TestCase):
    basic_event = mock_event.get_basic()
    schema_path = 'tests/mocks/sqs/openapi.yml'
    expected_body = {
        'lang': 'en-us',
        'sms': True,
        'email': True,
    ...
    def test_event_returns_data_class(self):
        event = Event(self.basic_event)
        event.data_class = MockSQSDataClass
        self.assertTrue(isinstance(event.records[0], MockSQSDataClass))
        self.assertTrue(isinstance(event.records[0].record, Record))

    def test_event_validate_record_body_with_schema_file(self):
        event = Event(self.basic_event, schema=self.schema_path, required_body='v1-sqs-body')
    ...
    def test_event_validate_filters_out_record_body_with_schema_file(self):
        event = Event(self.basic_event, schema=self.schema_path, required_body='v1-sqs-body-wrong')
        self.assertEqual(len(event.records), 0)

    def test_event_validate_raises_exception_record_body_with_schema_file(self):
        try:
            event = Event(self.basic_event, schema=self.schema_path, required_body='v1-sqs-body-wrong', raise_body_error=True)
            print(event.records)
