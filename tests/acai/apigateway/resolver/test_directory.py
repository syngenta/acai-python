...
class DirectoryResolverTest(unittest.TestCase):
    ...
    def test_basic_routing(self):
        ...
        file_path, import_path = self.directory_resolver.__get_file_and_import_path(request.path)
    ...
    def test_route_not_found_raises_resolver_exception(self):
        try:
            ...
            self.directory_resolver.__get_file_and_import_path(request.path)
            self.assertTrue(False)
        except ResolveException as resolver_error:
            self.assertTrue(isinstance(resolver_error, ResolveException))
