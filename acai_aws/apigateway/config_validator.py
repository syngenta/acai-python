...
class ConfigValidator:
    ...
    def validate(**kwargs):
        ...
        if kwargs.get('cache_size') and kwargs.get('cache_size') is not None and not isinstance(kwargs.get('cache_size'), int):
        ...
        if kwargs.get('cache_mode') and kwargs['cache_mode'] not in ('all', 'static-only', 'dynamic-only'):
            raise ApiException(code=500, message='cache_mode should be one of the following strings: all, static-only, dynamic-only')
